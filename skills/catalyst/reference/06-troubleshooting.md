...
| Native tools (Read / Edit / Bash / Write / Grep / etc.) are blocked with "use coding_workspace__*" | Expected — an App Building session is active. Use the redirect target the hook names. If the user genuinely needs native tools (they're editing local config, not the project), call `abandon_build` first to clear the session marker. |
