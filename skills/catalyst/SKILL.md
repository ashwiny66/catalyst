...
| `run_python` | Your notebook on the warehouse: pandas/numpy + a read-only `query(sql)` that returns a DataFrame straight from the DB. `df = query("SELECT …")`, then compute — cohorts, distributions, trends, correlations, outliers. |
