...
- **Sentinel is the single source of truth** for "is there an active Catalyst session, and which tab owns it":
  ...
  - Fields: `cc_session_id` (owner), `started_at`, `session_id` (catalyst build id), `app_root`, `gen_stream_id`, `mode` (`menu` | `coding` | `vibe_code` | `brainstorm` | `deep_analysis`).
...
- **Three native tools are always allowed for the owning tab during a build:** `TodoWrite`, `AskUserQuestion`, `Skill`, `SlashCommand`, `ToolSearch`, `EnterPlanMode`, `ExitPlanMode` ([catalyst-block-native.py:90](../../../codeGen/catalyst-plugin/hooks/catalyst-block-native.py#L90)). All other native FS/shell tools are denied with a redirect to `coding_workspace__*`. **EXCEPTION — `mode=deep_analysis`:** Case 3 returns `0` (allows) for ALL native tools (Deep Analysis is org-level research with no remote app workspace to protect). The block is otherwise mode-blind — every other live mode (menu/brainstorm/coding/vibe_code) blocks identically.
- **Deep Analysis (`mode=deep_analysis`) is a third, session-LESS mode** parallel to brainstorm/coding. Entered via the `start_analysis` lifecycle tool (writes `mode=deep_analysis`, `session_id=null`). It binds a read-only, org-scoped tool surface namespaced `analysis_workspace__*` (DB/API knowledge + `run_select_query` + uploads grep) that dispatches to the **session-less** builder route `POST /api/mcp/analysis/tool` (org resolved from the JWT, NOT a session). The PostToolUse event-sink is a no-op in this mode (Gate 2 only syncs coding/vibe_code) — research is ephemeral, no app row. Exit by building: `send_message`/`restart_brainstorm`/`switch_project` flip the mode off `deep_analysis` via the normal sentinel paths.
...
- **Subagents inherit the parent's `cc_session_id` and can SELF-CLAIM the sentinel** (observed 2026-05-31). PreToolUse only checks `cc_session_id` for ownership, not parent-vs-subagent. So when an Explore / general-purpose subagent (running under the parent's session) calls any `coding_workspace__*` MCP tool — which it might choose to do because the project's CLAUDE.md surfaces those tools prominently — the PreToolUse hook claims a fresh `mode=menu` sentinel under the parent's id. From that moment, the **subagent's own subsequent native `Read`/`Bash` calls are denied** with "Native X is blocked while a Catalyst session is active (mode=menu)", the subagent silently bails, and the parent sees an "Agent blocked" error. Mitigations: do exploration directly with native Read/Bash/Grep (no subagents) when working in catalyst-builder / enterprise_ai_developer_v2; OR append "Use only native Read/Bash/Grep — DO NOT use any `coding_workspace__*` or `mcp__plugin_catalyst_catalyst-mcp__*` tools (they will deadlock this subagent)" to the subagent prompt. Recovery: call `mcp__plugin_catalyst_catalyst-mcp__end` (tab-agnostic) to wipe the sentinel, then retry without subagents. The full debug recipe lives in [[feedback_explore_subagent_sentinel_trap]].
